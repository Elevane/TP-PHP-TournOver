...
<!--
nom :delCaroussel.php
fonction : ce fichier vise a supprimer un évenement
-->
...
// script de sécurité, si la personne ne s'est pas connecté, il est impossible d'acceder a la page , renvoies ensuite vers la page d'erreur (cf bas de page)
{
	...
	$fp = fopen('caroussel.csv', 'r'); // ouvrir le csv en lecture
		...
		while($data = fgetcsv($fp, 10000, ',')) // boucle qui parcours le fichier csv ligne par ligne, la virgule sépare les données
		{
			...
			{
				$liste2 = [$data[0],$data[1],$data[2],$data[3]]; // liste temporaire ou sont stockés les infos
	...
	$fp = fopen('caroussel.csv', 'w'); // ouvrir le csv en écriture
	...
	header('Location:gestionAcceuil.php'); // renvois au panneaux de gestion de contenu
